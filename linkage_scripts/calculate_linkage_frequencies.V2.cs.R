...
# Master df
cor_df <- fread("results/linkage_out/linkage.n30.rsquared25.csv")
...
link_morsels <- foreach(j = seq(nrow(cor_filt))) %do% {
  ...
  mat_temp <- mat[, c(row$mutation1, row$mutation2)]
...
fwrite(temp, str_glue("results/linkage_out/monthly_linkage/linkage_freq.{col_month}.csv"))

...
df <- fread("results/allele_frequency_out/all_sites.csv") %>%
  left_join(hookup %>% select(mutation_name, ref_AA, var_AA))
...
cor.test(merged$max_freq, merged$global_n)
...
ggsave("results/allele_frequency_out/blosum62_versus_global_n.png", width = 4, height = 4)

merged %>%
  ggplot(aes(x = norm_sd_freq, y = log10(global_n))) +
  geom_point(color = "steelblue") +
  geom_smooth(method = "glm", method.args = list(family = poisson), color = "black", fill = "orange") +
  theme_bw() +
  labs(x = "CoV(freq.)", y = "Log10(no. GISAID sequences)")

ggsave("results/allele_frequency_out/CoV_versus_global_n.png", width = 4, height = 4)

# Model global counts
preg <- glm(global_n ~ norm_sd_freq + norm_n + blosum62_score + median_freq + max_freq,
            family = poisson(),
            data = merged)

...
ggsave("results/allele_frequency_out/max_freq_versus_global_n.significant.pdf", 
...
ggsave("results/allele_frequency_out/n_versus_global_n.siginficant.pdf", 
       plot = p2, width = 4, height = 4)

cor_test <- cor.test(merged$abs_charge, merged$global_n, method = "spearman")
rho <- signif(cor_test$estimate, 2)

p3 <- merged %>%
  ggplot(aes(x = abs_charge, y = log10(global_n))) +
  geom_bin2d() +
  scale_fill_viridis_c() +
  geom_smooth(method = "lm", color = "black", fill = "grey") +
  theme_bw() +
...
#   annotate("text", x = 0, y = 7, label = str_glue("rho={rho}, p<0.0001"))
# 
# ggsave("results/allele_frequency_out/CoV_versus_global_n.significant.pdf", 
#        plot = p3, width = 4, height = 4)

ggarrange(p1, p2, p3, nrow = 1)
ggsave("results/allele_frequency_out/sig_predictors.significant.pdf", 
       width = 12, height = 4)
# merged %>%
...
tibble(var = c("median freq", "max freq", "n", "CV", "charge", "mw", "hydropathy", "blosum62"),
  ...
  mutate(rho = signif(rho, 2), 
         padj = signif(p.adjust(pval, method = "BH"), 2)) %>% 
  filter(var != "CV")
